...
import {
  type OrganizerNews, listOrganizerNews, withdrawOrganizerNews,
...
const statusLabel: Record<OrganizerNews["status"], string> = {
  submitted: "待审核", rejected: "已驳回", published: "已发布", withdrawn: "已撤回",
}

export function ConferenceNewsWorkspace({ token, conferenceId, lifecycle, onError }: Props) {
  ...
  if (lifecycle === "draft") {
    return <Card><CardContent className="py-10 text-center text-muted-foreground">大会激活后可查看新闻记录。</CardContent></Card>
  }

  return <section className="space-y-4">
    <div className="flex items-center justify-between gap-3">
      <div><h2 className="font-semibold">新闻管理</h2><p className="text-sm text-muted-foreground">组织者可以查看全部稿件与附原因撤回，不能代发或审核。</p></div>
      <Button variant="outline" disabled={loading} onClick={() => void load()}>
        {loading ? <Loader2 className="animate-spin" /> : <RefreshCw />}刷新
      </Button>
    </div>
    {news.length === 0 ? <Card><CardContent className="py-10 text-center text-muted-foreground">暂无新闻记录</CardContent></Card> : news.map((item) =>
      <Card key={item.id}>
        <CardHeader><div className="flex items-start justify-between gap-4">
          <div><CardTitle>{item.title}</CardTitle><p className="mt-1 text-sm text-muted-foreground">{item.source} · {formatTime(item.contentTime)} · {item.author.committeeName} / {item.author.seatName}</p></div>
        ...
        <CardContent className="space-y-3">
          ...
          {item.reviewNote && <p className="rounded-md bg-muted p-3 text-sm">审核说明：{item.reviewNote}</p>}
          {item.withdrawalReason && <p className="rounded-md bg-destructive/10 p-3 text-sm text-destructive">撤回原因：{item.withdrawalReason}</p>}
          {lifecycle === "active" && item.status === "published" && <div className="border-t pt-3">
            {withdrawId === item.id ? <div className="space-y-2">
              <Label htmlFor={`news-withdraw-${item.id}`}>撤回原因</Label>
              <textarea id={`news-withdraw-${item.id}`} value={reason} maxLength={500} rows={3} onChange={(event) => setReason(event.target.value)} className="w-full rounded-md border border-input bg-transparent p-3 text-sm" />
